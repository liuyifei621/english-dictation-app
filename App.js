...
export default function App() {
  ...
  useEffect(() => {
    AsyncStorage.getItem('english-dictation:last-library').then((value) => {
      if (value) {
        ...
        if (saved.sources?.length) { setSources(saved.sources); setEntries(saved.sources.flatMap((source) => source.entries)); }
  ...
  const start = () => {
    ...
    setSessionEntries(list); setIndex(0); setRunning(true); setPaused(false); setAnswerVisible(false); setShowAllAnswers(false); setRevealedCount(0); activeRef.current = true;
